...
import { DEFAULT_REPLY } from "../consts";
...
export function CommentItemView(prop: { comment: CommentWithAction }) {
  const [menuPos, setMenuPos] = createSignal<{ x: number; y: number } | null>(
    null,
  );
  ...
  return (
    <div
      title={
        prop.comment.isSubmitted
          ? "This comment has been submitted and cannot be edited."
          : ""
      }
        ...
        <div class="flex flex-col w-full h-full">
          <div class="flex space-x-2">
            <div class="flex flex-col w-full space-y-1">
              ...
              >
                <div>
                  {prop.comment.authorName} ·{" "}
                  {timeAgo(prop.comment.createdAtCommunityTime)}
                </div>
              ...
              {/* Comment Content */}
              <div
                class="leading-[18px] text-grey-channel-text w-fit"
                innerHTML={prop.comment.content}
                onClick={(e) => e.stopPropagation()}
              />
            </div>
            {/* Action Button */}
          ...
          {isReplyAvailableOrOngoing(prop.comment) && (
            ...
            >
              <textarea
                ...
                onInput={(e) =>
                  setRepliesStore(prop.comment.id, e.currentTarget.value)
                }
                onBlur={() => {
              ...
              >
                <img src="/buttonClose.svg" class="w-[8px] h-auto" />
              </button>
            </div>
          )}
        </div>
        ...
        >
          <div>
            ...
            <span class="pl-1">
              {(() => {
                const actions = [
                  hasAction(prop.comment, "Reply") ? "1 Reply" : null,
                  hasAction(prop.comment, "Like") ? "1 Like" : null,
                  hasAction(prop.comment, "Dislike") ? "1 Dislike" : null,
                  hasAction(prop.comment, "Delete") ? "1 Delete" : null,
