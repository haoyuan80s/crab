...
export const [displayPostWithCrawlIds, setDisplayPostWithCrawlIds] =
  createSignal<string[]>();

// Derived based on postWithCrawlResource. The first two are not mutable and the last one is mutable.
export const [channelRenderInfos, setChannelRenderInfos] = createSignal<
  ChannelRenderInfo[]
>([]);
export const [postRenderInfos, setPostRenderInfos] = createSignal<
  PostRenderInfo[]
>([]);
export const [commentsStore, setCommentsStore] = createStore<
  CommentWithAction[]
>([]);

// Manage reply text in textarea: comment_id => reply string.
export const [repliesStore, setRepliesStore] = createStore<
...
export const editPostWatch = (postId: string, isActive: boolean) => {
  ...
  if (isActive) {
    ...
    userWatchStore.inactivePostIds = userWatchStore.inactivePostIds.filter(
      (id) => id !== postId,
    );
  } else {
    userWatchStore.activePostIds = userWatchStore.activePostIds.filter(
      (id) => id !== postId,
    );
...
export const getCommnentsOfActionType = (
  postId: string,
  actionType: CommentActionType,
): CommentWithAction[] => {
  return commentsStore.filter((comment) =>
    comment.actions.some(
      (action) => action.type === actionType && comment.postId === postId,
    ),
  );
};

export const countCommentsOfActionTYpe = (
  postId: string,
  actionType: CommentActionType,
): number => {
...
export const getCommentsWithAnyAction = (
  postId: string,
): CommentWithAction[] => {
  return commentsStore.filter(
    (comment) => comment.postId === postId && comment.actions.length > 0,
  );
};
...
export const getCommentsWithNoAction = (
  postId: string,
): CommentWithAction[] => {
  return commentsStore.filter(
    (comment) => comment.postId === postId && comment.actions.length === 0,
...
export const countCommentsByTab = (postId: string, tab: string): number => {
  ...
  if (tab === "AllReact") {
    return (
      countCommentsOfActionTYpe(postId, "Like") +
      countCommentsOfActionTYpe(postId, "Dislike") +
      countCommentsOfActionTYpe(postId, "Reply") +
      countCommentsOfActionTYpe(postId, "Delete")
    );
  }
  return -1;
};

export function extractCommentsFromPostWithCrawlResponse(
  response: PostWithCrawlResponse[],
): CommentWithAction[] {
...
export function extractPostRenderInfosFromPostWithCrawlResponse(
  response: PostWithCrawlResponse[],
): PostRenderInfo[] {
...
export function extractChannelRenderInfosFromPostWithCrawlResponse(
  response: PostWithCrawlResponse[],
): ChannelRenderInfo[] {
